...
class SessionManager:
    ...
    SESSION_ID_SUFFIX_RE = re.compile(r"_(?:[a-f0-9]{8})$")
    ...
    def _get_session_path(self, full_session_id: str) -> Path:
        ...
        safe_session_id = full_session_id
    ...
    def _split_session_id(self, full_session_id: str) -> Tuple[str, Optional[str]]:
        ...
        if match:
            ...
            uuid_part = full_session_id[match.start() + 1 :]
    ...
    def _generate_full_session_id(self, display_name: str) -> str:
        ...
        sanitized_name = sanitized_name or "session"
        while True:
    ...
    def rename(
        self, old_full_session_id: str, new_display_name: str
    ) -> Optional[str]:  # Return new ID or None
        """Renames a session file, preserving the UUID suffix. Returns new full ID on success."""
        ...
        if uuid_suffix is None:
            ...
            # print(f"[yellow]Warning: Old session ID '{old_full_session_id}' did not have standard suffix. Generating new ID.[/yellow]") # Optional Warning
        else:
        ...
        if old_full_session_id == new_full_id:
            # print("[yellow]New name is the same as the old name. No changes made.[/yellow]") # Optional Warning
    ...
    def save_messages(self, full_session_id: str, messages: List[Dict[str, str]]):
        """Saves the entire message list to the session file."""
        path = self._get_session_path(full_session_id)
        try:
            with path.open("w") as f:
    ...
    def load_messages(self, full_session_id: str) -> List[Dict[str, str]]:
        """Loads messages from a session file."""
        path = self._get_session_path(full_session_id)
        if not path.exists():
    ...
    def append_message(self, full_session_id: str, role: str, content: str):
        """Appends a single message and saves the session."""
    ...
    def list_sessions(self) -> List[Tuple[str, str]]:
        ...
        sorted by last modification time (latest first).
        Filters out the _{uuid} suffix for the display name.
        """
        sessions_with_time = []
        try:
            # Iterate through all json files in the session directory
            for session_file_path in self.session_dir.glob("*.json"):
                if session_file_path.is_file():
                    try:
                        # Get modification time
                        mtime = session_file_path.stat().st_mtime
                        # Get full id (filename without extension)
                        full_id = session_file_path.stem
                        # Get display name
                        display_name, _ = self._split_session_id(full_id)
                        # Store relevant info
                        sessions_with_time.append((display_name, full_id, mtime))
                    except OSError:
                        # Skip files if stat fails (e.g., permission error)
                        # You could log this error if needed
                        continue
                    except ValueError:
                        # Skip files if splitting the name fails (e.g., unexpected format)
                        continue

            # Sort the list by modification time (the 3rd element, index 2), descending order
            sessions_with_time.sort(key=lambda item: item[2], reverse=True)

            # Return only the display name and full ID from the sorted list
            return [(disp_name, f_id) for disp_name, f_id, mtime in sessions_with_time]

        except OSError as e:
            # Handle error during the initial directory listing (glob)
            raise SessionError(
                f"Error listing sessions directory {self.session_dir}: {e}"
            )

    # --- END MODIFIED list_sessions ---
